...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(input_size=embed_size, 
                               ...
                               batch_first=False,
                               bidirectional=True)
        # Decoder of the NMT model
        self.decoder = nn.LSTMCell( input_size=embed_size+hidden_size,
                                    hidden_size=hidden_size,
                                    bias=True)
        # Projection layer for the hidden states
        self.h_projection = nn.Linear(2*hidden_size, hidden_size, bias=False)
        # Projection layer for the cell states
        self.c_projection = nn.Linear(2*hidden_size, hidden_size, bias=False)
        # Projection layer for the attention
        self.att_projection = nn.Linear(2*hidden_size, hidden_size, bias=False)
        # Projection layer for the combined output
        self.combined_output_projection = nn.Linear(3*hidden_size, hidden_size, bias=False)
        # Projection layer for the target vocabulary
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # Pad the packed hidden states
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)
        # Reshape enc_hiddens to (b, src_len, h*2)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)
        # concatenate forwards and backwards hidden states
        init_decoder_hidden = torch.cat((last_hidden[0], last_hidden[1]), dim=1)
        # appy projection layer to get initial hidden state for decoder
        init_decoder_hidden = self.h_projection(init_decoder_hidden)
        # concatenate forwards and backwards cell states
        init_decoder_cell = torch.cat((last_cell[0], last_cell[1]), dim=1)
        # apply projection layer to get initial cell state for decoder
        init_decoder_cell = self.c_projection(init_decoder_cell)
        # unpack decoder initial states
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # construct Y of target sentences
        Y = self.model_embeddings.target(target_padded)
        # iterate over the time dimension of Y
        for Y_t in torch.split(Y, 1, dim=0):
            # squeeze Y_t
            Y_t = torch.squeeze(Y_t, dim=0)
            # construct Ybar_t by concatenating Y_t with o_prev on their last dimension
            Ybar_t = torch.cat((Y_t, o_prev), dim=1)
            # compute the Decoder's next (cell, state) values
            dec_state, o_t, e_t = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            # append o_t to combined_outputs
            combined_outputs.append(o_t)
            # update o_prev to the new o_t
            o_prev = o_t
        # stack combined_outputs
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # Apply the decoder to target embedding and previous hidden and cell states
        dec_state = self.decoder(Ybar_t, dec_state)
        # Split dec_state into its two parts (dec_hidden, dec_cell)
        (dec_hidden, dec_cell) = dec_state
        # Compute the attention scores e_t
        ...
        # get attention output vector
        a_t = torch.bmm(torch.unsqueeze(alpha_t, dim=1), enc_hiddens)
        a_t = torch.squeeze(a_t, dim=1)
        # concatenate dec_hidden with a_t
        U_t = torch.cat((dec_hidden, a_t), dim=1)
        # apply combined output projection layer
