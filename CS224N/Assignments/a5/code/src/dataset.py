...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        document_truncated_length = random.randint(4, int(self.block_size*7/8))
        # truncate the document to the selected length
        document = document[:document_truncated_length]
        # randomly select a length for the mask to have an average of 1/4 of the document length using gaussian distribution
        mask_length = int(random.gauss(document_truncated_length/4, document_truncated_length/16))
        # clip the mask length to be within the document length
        mask_length = max(1, min(mask_length, document_truncated_length))
        # randomly select a start index for the mask
        mask_start_index = random.randint(0, document_truncated_length-mask_length)
        # split the document into three parts: [prefix, masked_content, suffix]
        prefix, masked_content, suffix = \
            document[:mask_start_index], \
            document[mask_start_index:mask_start_index+mask_length], \
            document[mask_start_index+mask_length:]
        # sanity check
        assert document == prefix + masked_content + suffix

        # rearrange these substrings as: [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] MASK_CHAR [pads]
        rearranged_substrings = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.MASK_CHAR
        # pad the rearranged_substrings to the block_size
        masked_string = \
            rearranged_substrings + self.PAD_CHAR * (self.block_size - len(rearranged_substrings))

        # build input and output strings for the model
        input_string = masked_string[:-1]
        output_string = masked_string[1:]

        # encode both strings and convert to long tensors
