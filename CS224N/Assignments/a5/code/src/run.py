...
setattr(mconf, 'synthesizer', args.variant == 'synthesizer')
...
if args.function == 'pretrain':
    ...
    # instantiate char corruption dataset as the pretrain dataset
    pretrain_dataset = dataset.CharCorruptionDataset(
                            open(args.pretrain_corpus_path).read(), block_size)
    ...
    # instantiate trainer
    trainer = trainer.Trainer(model, pretrain_dataset, None, tconfig)
    # train the model
    trainer.train()
    # save the model
    trainer.save_checkpoint()
    print('pretraining complete - saved the model to {}'.format(args.writing_params_path))
    ...
    if args.reading_params_path is not None:
        ...
        print('loaded checkpoint from {}'.format(args.reading_params_path))
        # trainer config parameters
        tconfig = trainer.TrainerConfig(
            max_epochs=10,
            batch_size=256,
    ...
    # define finetune dataset
    finetune_dataset = dataset.NameDataset(pretrain_dataset,
                                           open(args.finetune_corpus_path).read())
    
    # instantiate the trainer module
    trainer = trainer.Trainer(model, finetune_dataset, None, tconfig)
    # train the model
    trainer.train()
    # save the model
    trainer.save_checkpoint()
    print('finetuning complete - saved the model to {}'.format(args.writing_params_path))
