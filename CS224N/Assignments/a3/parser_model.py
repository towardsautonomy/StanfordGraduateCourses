...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # define the weight matrix for the embedding to hidden layer
        self.embed_to_hidden_weight = nn.Parameter(
                torch.zeros(self.n_features*self.embed_size, self.hidden_size))
        # define the bias for the embedding to hidden layer
        ...
        # define the weight matrix and bias vector for the hidden to logits layer
    ...
    def embedding_lookup(self, w):
        ...
        w = w.to(self.device)
        x = torch.index_select(self.embeddings, 0, w.view(-1)).view(w.shape[0], -1).to(self.device)
    ...
    def forward(self, w):
        ...
        h = nn.ReLU()(torch.matmul(self.embedding_lookup(w), self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        h = self.dropout(h)
